...
int output_format = OUTPUT__FORMAT_BINARY; //OUTPUT__FORMAT_TEXT;
...
#define ACCEL_Y_MAX ((float) 283)
#define ACCEL_Z_MIN ((float) -280)
#define ACCEL_Z_MAX ((float) 300)
...
#define GYRO_AVERAGE_OFFSET_X ((float) -0.1)
#define GYRO_AVERAGE_OFFSET_Y ((float) 43.1)
#define GYRO_AVERAGE_OFFSET_Z ((float) -23.6)
...
void setup()
{
  ...
  // Init sensors
  delay(150);  // Give sensors enough time to start
  I2C_Init();
  Accel_Init();
  Magn_Init();
  Gyro_Init();
  ...
  delay(120);  // Give sensors enough time to collect data
  reset_sensor_fusion();
